...
#' the form c(kappa,sigma,gamma,rho). For intrinsic models, the vector should be
#' c(tau,kappa) or c(tau) depending on if kappa is in the model. 
...
#' is not given for non-intrinsic models. Not used for spatio-temporal models.
#' @param start_nu Starting value for the smoothness parameter of spatial non-intrinsic models.
...
#' @param beta If `NULL`, the smoothness parameter will be estimated for spatial
#' intrinsic models, otherwise it is kept fixed at the provided value.
#' @param start_beta Starting value for beta parameter for intrinsic models.
...
rspde_lme <- function(formula, 
  ...
  if (!is.null(model)) {
    ...
    if(!spacetime && !anisotropic && !intrinsic) {
  ...
  if(is.character(repl)) {
      if(repl %in% colnames(data)) {
          ...
          stop("repl provides as a name of an element in data, but the name does not exist.")
      }
  ...
  if (!null_model) {
    ...
    if (is.null(starting_values_latent)) {
        ...
        starting_values_latent <- model$theta
          ...
          if(anisotropic) {
              ...
              if(!estimate_alpha && alpha == 0) {
                  starting_values_latent <- log(c(model$tau))      
              } else {
                  starting_values_latent <- log(c(model$tau, max(c(model$kappa, 1e-5))))      
              }
          } else {
              starting_values_latent <-  log(c(model$tau, max(c(model$kappa, 1e-5))))      
          }
      ...
      if (model$stationary && !spacetime) {
          if(anisotropic) {
              ...
              if(!estimate_alpha && alpha == 0 && length(starting_values_latent) != 1) {
                  stop("starting_values_latent must be a vector of length 1.")
              } 
              if (estimate_alpha && length(starting_values_latent) != 2) {
    ...
    if (intrinsic) {
        ...
        if(estimate_alpha && estimate_beta) {
            ...
            start_values <- c(log(0.1 * sd(y_resp)), beta2theta(start_beta, model$d), starting_values_latent)
        } else {
            start_values <- c(log(0.1 * sd(y_resp)), starting_values_latent)    
        }
    } else if (estimate_nu && !spacetime) {
