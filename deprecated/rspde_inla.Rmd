...
and to [@xiong22](https://arxiv.org/abs/2209.04670).
...
2. To this end we can use the `rspde.make.A()` function. Since we will assume order
...
4. Other common choices for `nu.upper.bound` are 2 and 1.
...
in Paraná region in Brazil and consider `nu.upper.bound` equal to 2, which is generally
a good choice for `nu.upper.bound`.

We simply use the function `rspde.matern()` with the argument `nu.upper.bound` set to 2:
...
The default `rspde.order` of 2 is generally a good choice and reasonably
...
Let us compare the cost of having `rspde.order=3` and 
`nu.upper.bound=2` with the cost of having `rspde.order=2` and `nu.upper.bound=4`:
...
Let us now see the summary of the fit in the original scale:
...
The following figure shows the posterior marginal densities of the three parameters:
...
default `rspde.order` (which is 2):
...
```{r}
rspde_model_brasil <- rspde.matern(prmesh, rspde.order = 1,
              type.rational.approx = "brasil")

f.s.1.brasil <- y ~ -1 + Intercept + f(seaDist, model = "rw1") +
  f(field, model = rspde_model_brasil)

rspde_fit_order_1_brasil <- inla(f.s.1.brasil,
  family = "Gamma", data = inla.stack.data(stk.dat.1),
  verbose = FALSE,
  control.inla = list(int.strategy = "eb"),
  control.predictor = list(A = inla.stack.A(stk.dat.1), compute = TRUE),
