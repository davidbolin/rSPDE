...
//   double *coeff;
  ...
  case INLA_CGENERIC_Q:
    ...
    double *coeff, *Bmat, *tmp_vec, *tmp_mat;

      coeff = Calloc(size, double);
      Bmat = Calloc(SQR(size), double);
      tmp_vec = Calloc(size, double);
      tmp_mat = Calloc(SQR(size), double);

    double L = alpha - floor(alpha);
    for (i = 0; i < size; i++){
        coeff[i] = 2*kappa_integral(i,-alpha+2.0*(size-1)+1.0+L,kappa);
    for (j = 0; j < size; j++){
