...
double *inla_cgeneric_rspde_nonstat_int_model(inla_cgeneric_cmd_tp cmd, double *theta, inla_cgeneric_data_tp * data) {
  ...
  assert(!strcasecmp(data->doubles[0]->name, "start.theta"));
  inla_cgeneric_vec_tp *start_theta = data->doubles[0];
  ...
  case INLA_CGENERIC_Q:
    {
      ...
      compute_Q_integer(n_mesh, C->x, C->i, C->j,
                        C->n,
                        G->x, G->i, G->j,
                        G->n,
                        B_kappa->x, B_tau->x,
