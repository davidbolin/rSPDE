...
For this example we set `nu_upper_bound=2`.
...
ggplot(field.pred.sd.df, aes(x = x1, y = x2, fill = y)) +
  geom_raster()
```
...
```{r plot_pred_pxl, fig.align='center'}
pxl <- pixels(mesh_2d)
...
To simplify parameter estimation, `rSPDE.construct.matern.loglike()` is a function
factory and returns the negative of log-likelihood function, parametrized using the logarithm of each 
...
ggplot(field.pred2.sd.df, aes(x = x1, y = x2, fill = y)) +
  geom_raster()
```
