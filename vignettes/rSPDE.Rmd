...
rspde_fitprd <- rspde.inla(f, family = "Gaussian", 
...
p <- ggplot(field.pred.df,aes(x=x1,y=x2,fill=y))+geom_raster()+scale_fill_gradient(low="yellow", high="red")
tryCatch({print(p)}, error=function(e) {
      print('Unable to build the plot')
    })
```
...
p <- ggplot(field.pred.sd.df,aes(x=x1,y=x2,fill=y))+geom_raster()
tryCatch({print(p)}, error=function(e) {
      print('Unable to build the plot')
    })
```
...
p <- ggplot(field.pred2.df,aes(x=x1,y=x2,fill=y))+geom_raster()+scale_fill_gradient(low="yellow", high="red")
tryCatch({print(p)}, error=function(e) {
      print('Unable to build the plot')
    })
```
...
p <- ggplot(field.pred2.sd.df,aes(x=x1,y=x2,fill=y))+geom_raster()
tryCatch({print(p)}, error=function(e) {
      print('Unable to build the plot')
    })
```
