...
* The `r inla_link()` implementation of the covariance-based rational SPDE approach [@xiong22](https://arxiv.org/abs/2209.04670);

* The rational SPDE approach of [@bolin19](https://www.tandfonline.com/doi/full/10.1080/10618600.2019.1665537).
...
* [R-INLA implementation of the rational SPDE approach](inla.html)
...
See the  [R-INLA implementation of the rational SPDE approach](inla.html)
for further details.
...
We can directly create the model object. Just as in `R-INLA`'s case, we need to set
an upper bound for the smoothness parameter $\nu$. When
we increase the upper bound for $\nu$
we also increase the computational cost. For this example we set `nu_upper_bound=1`.
See the  [R-INLA implementation of the rational SPDE approach](rspde_inlabru.html)
for further details.
...
## Kringing with `inlabru` implementation of the rational SPDE approach

Let us now obtain predictions (i.e., do kriging) of the latent field on 
a dense grid in the region.

We begin by creating the grid in which we want to do the predictions. We begin
by creating a regular grid in which we want to do precictions:
```{r}
grd <- expand.grid(1:100, 1:100) / 100
```

We will now use extract the coordinates of the previous grid:
...
p <- ggplot(field_pred_df, aes(x = x1, y = x2, fill = mean)) +
  geom_raster() +
  scale_fill_gradient(low = "yellow", high = "red")
p
```
...
p <- ggplot(field_pred_df, aes(x = x1, y = x2, fill = sd)) +
  geom_raster()
p
```
...
Let us now calculate the predictions. To this end we will use the `predict()`
method in `inlabru`:
...
```{r plot_pred_pxl, echo=FALSE, fig.align='center'}
...
parameter to avoid constrained optimization. This objective function is
created by the function factory `rSPDE.construct.matern.loglike()` :
...
results <- data.frame(
  kappa = c(kappa, exp(pars$par[1])),
  sigma = c(sigma, exp(pars$par[2])),
