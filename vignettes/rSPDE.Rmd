...
use the `r inla_link()` function `fm_basis()` to construct the observation matrix which can be used to evaluate the
...
rspde_fit <-
  inla(f,
    ...
    family = "gaussian",
    control.predictor =
      list(A = inla.stack.A(st.dat)), debug=TRUE, verbose=TRUE
  )
```
