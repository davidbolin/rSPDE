...
p <- tryCatch({ggplot(df,aes(x=x1,y=x2,col=y))+geom_point()+scale_color_gradient(low="yellow", high="red")}, error = function(e){print("Could not run the example")})
p
```
...
p <- tryCatch({ggplot(field.df,aes(x=x1,y=x2,fill=y))+geom_raster()+scale_fill_gradient(low="yellow", high="red")}, error = function(e){print("Could not run the example")})
p
```
