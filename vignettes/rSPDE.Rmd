...
p <- ggplot(df,aes(x=x1,y=x2,col=y))+geom_point()+scale_color_gradient(low="yellow", high="red")
tryCatch({p}, error = function(e){print("Could not render the plot")})
```
...
p<- ggplot(field.df,aes(x=x1,y=x2,fill=y))+geom_raster()+scale_fill_gradient(low="yellow", high="red")
tryCatch({p}, error = function(e){print("Could not render the plot")})
```
