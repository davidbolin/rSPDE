...
```{r plot_precipitations, message=FALSE,fig.height=4,fig.width=7}
...
```{r plot_prec_as_func, message=FALSE, fig.width=7, fig.height=6}
par(mfrow = c(2, 2))
...
```{r mesh_creation, message=FALSE, fig.width=7, fig.height=5}
...
As argued in `INLA`'s man page, not all features are available in
...
```{r plot_post, fig.width=7, fig.height=4}
par(mfrow=c(1,3))
plot(result_fit)
```
...
```{r plot_prd, fig.height=7, fig.width=7}
...
Since we are doing predictions, we should not use `INLA`'s experimental mode.
...
```{r plot_pred, fig.width=7, fig.height=14}
...
```{r plot_post_order_3, fig.height=4, fig.width=7}
par(mfrow=c(1,3))
...
We also assume that, in terms of INLA's parameters,
$$\nu = \nu_{UB}\Big(\frac{\exp(\theta_3)}{1+\exp(\theta_3)}\Big).$$
...
`INLA`'s counterparts, namely, `inla.spde.make.A()` and `inla.make.index()`.
