...
rspde_model_order_3 <- rspde.matern(
...
rspde_fit_order_3 <- rspde.inla(f.s.3,
...
```{r message=FALSE}
rspde_model_fix_int1 <- rspde.matern(
  mesh = prmesh,
  nu = 1
)
```
...
rspde_fix_int_1 <- rspde.inla(f.s.fix.int.1,
  family = "Gamma",
...
rspde_fit_ln <- rspde.inla(f.s.ln,
  family = "Gamma", data = inla.stack.data(stk.dat),
  verbose = FALSE,
  control.inla = list(int.strategy = "eb"),
  control.predictor = list(A = inla.stack.A(stk.dat), compute = TRUE)
...
```{r}
rspde_model_order_3_start <- rspde.matern(
  mesh = prmesh, rspde_order = 3,
  nu_upper_bound = 2,
...
rspde_fit_order_3_start <- rspde.inla(f.s.3.start,
  family = "Gamma",
...
start_time <- Sys.time()
rspde_model_opt_sharp <- rspde.matern(
  mesh = prmesh,
  rspde_order = 1
)
end_time <- Sys.time()
...
start_time <- Sys.time()
rspde_model_opt_notsharp <- rspde.matern(
  mesh = prmesh,
  rspde_order = 1,
  sharp = FALSE
)
...
start_time <- Sys.time()
rspde_model_nonopt <- rspde.matern(
  mesh = prmesh,
  rspde_order = 1,
  optimize = FALSE
)
...
rspde_fit_opt_sharp <- rspde.inla(f.s.opt.sharp,
  family = "Gamma",
...
rspde_fit_opt_notsharp <- rspde.inla(f.s.opt.notsharp,
  family = "Gamma",
  data = inla.stack.data(stk.dat.1),
  verbose = FALSE,
  control.inla = list(int.strategy = "eb"),
...
rspde_fit_nonopt <- rspde.inla(f.s.nonopt,
  family = "Gamma",
  data = inla.stack.data(stk.dat.1),
  verbose = FALSE,
  control.inla = list(int.strategy = "eb"),
...
```{r}
rspde_model.rep <- rspde.matern(mesh = mesh_2d, type.rational.approx = "brasil")
```
