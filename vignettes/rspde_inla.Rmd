...
`rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` in place of `inla.spde2.matern()`. To create an index, one should use
...
Therefore, to set up a model all we have to do is use the `rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` function:

```{r create_model, message=FALSE}
rspde_model <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),mesh = prmesh)
```
...
grid.arrange(
  levelplot(m.prd,
    col.regions = tim.colors(99),
    xlab = "", ylab = "", main = "mean",
    scales = list(draw = FALSE)
  ),
  levelplot(sd.prd,
    col.regions = topo.colors(99),
...
When we fit a `rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` model we need to provide an upper bound
...
We simply use the function `rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` with the argument `nu_upper_bound` set to 2:

```{r create_model_ub}
rspde_model_2 <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),mesh = prmesh, nu_upper_bound = 2)
```
...
rspde_model_order_3 <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh, rspde_order = 3,
  nu_upper_bound = 2
)
```
...
```{r message=FALSE}
rspde_model_fix <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh, rspde_order = 2,
  nu = 1.21
)
```
...
```{r message=FALSE}
rspde_model_fix_int1 <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh,
  nu = 1
)
```
...
We have that
$$\tau = \exp(\theta_1),\quad \kappa = \exp(\theta_2).$$
The `rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` function assumes a lognormal prior distribution for $\tau$ and $\kappa$. This prior distribution is obtained by assuming
...
changing the argument `nu.prec.inc` in the `rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` function. The higher
...
```{r}
rspde_model_ln <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh, prior.nu.dist = "lognormal",
  nu_upper_bound = 2
)
```
...
```{r}
rspde_model_order_3_start <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh, rspde_order = 3,
  nu_upper_bound = 2,
...
The `rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),)` function has an optimized version, which is the default
...
start_time <- Sys.time()
rspde_model_opt_sharp <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh,
  rspde_order = 1
)
end_time <- Sys.time()
...
start_time <- Sys.time()
rspde_model_opt_notsharp <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh,
  rspde_order = 1,
  sharp = FALSE
)
...
start_time <- Sys.time()
rspde_model_nonopt <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),
  mesh = prmesh,
  rspde_order = 1,
  optimize = FALSE
)
...
rspde_fit_opt_sharp <- inla(f.s.opt.sharp,
  family = "Gamma",
  data = inla.stack.data(stk.dat.1),
  verbose = FALSE,
  control.inla = list(int.strategy = "eb"),
  control.predictor = list(
    A = inla.stack.A(stk.dat.1),
    compute = FALSE
  )
)
...
rspde_fit_nonopt <- inla(f.s.nonopt,
  family = "Gamma",
  data = inla.stack.data(stk.dat.1),
  verbose = FALSE,
  control.inla = list(int.strategy = "eb"),
...
We now create the model object. For illustration purposes, we will use "BRASIL" 
rational approximation algorithm to fit this model.

```{r}
rspde_model.rep <- rspde.matern(prior.tau=list(sdlog=sqrt(10)), prior.kappa=list(sdlog=sqrt(10)),mesh = mesh_2d, type.rational.approx = "brasil")
```
