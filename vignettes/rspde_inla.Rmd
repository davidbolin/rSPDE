...
It is important to mention that our implementation in the R-INLA package
requires the PARDISO solver. Please, go to (https://www.pardiso-project.org/r-inla/#license)
...
We can use usa INLA for creating the mesh. Let us create a mesh which is based 
...
```{r message=FALSE}
rspde_model <- create_rspde_model(inla_mesh = prmesh, d=2)
```
...
```{r message=FALSE}
rspde_model_3 <- create_rspde_model(inla_mesh = prmesh, d=2, rspde_order = 3)
```
...
We can fix the smoothness, say $\nu$, of the model by setting the argument `fixed_nu` to
`TRUE` *and* provide the value of $\nu$.
...
`fixed_nu` to `TRUE` and set `nu` to `0.78`:

```{r message=FALSE}
rspde_model_fix <- create_rspde_model(inla_mesh = prmesh, d=2, rspde_order = 2,
                                      fixed_nu = TRUE, nu = 0.78)
```
...
```{r message=FALSE}
rspde_model_fix_3 <- create_rspde_model(inla_mesh = prmesh, d=2, rspde_order = 3,
                                      fixed_nu = TRUE, nu = 0.78)
...
create a new model (remember to set `fixed_nu=TRUE` and `nu=1`):

```{r message=FALSE}
rspde_model_fix_int1 <- create_rspde_model(inla_mesh = prmesh, d=2,
                                      fixed_nu = TRUE, nu = 1)
```
...
```{r message=FALSE}
rspde_model_fix_int2 <- create_rspde_model(inla_mesh = prmesh, d=2,
                                           fixed_nu = TRUE, nu = 2)
