...
In this vignette we will present the R-INLA implementation of the rational
SPDE approach. For theoretical details we refer the reader to 
the [Covariance-based rational approximation with the `rSPDE` package](rspde_cov.html) vignette.
...
INLA uses the following parameterization of the Gamma distribution, 
...
```{r plot_precipitations, message=FALSE}
...
```{r plot_prec_as_func, message=FALSE, fig.width=7.5, fig.height=6}
par(mfrow = c(2, 2))
...
The rSPDE-INLA implementation is very reminiscent of INLA, so its usage should be
straightforward for INLA users. For instance, to create a rSPDE model, one would
`rpsde.matern()` in place of `inla.spde2.matern()`. To create an index, one should use
...
and the standard SPDE implementation in INLA, are the `nu` and `rspde_order` arguments,
...
of interest (more triangles = larger computation times):

```{r mesh_creation, message=FALSE, fig.width=14, fig.height=9}
...
For this task, as we mentioned earlier, we need to use a rSPDE specific function, 
whose name is very reminiscent to INLA's standard SPDE approach,
namely `rspde.make.A()` (in place of INLA's `inla.spde.make.A()`). 
...
can be found in the introduction of this vignette.

The default order is 2 for our covariance-based rational approximation. 
As mentioned in the introduction, an approximation of order 2 in the covariance-based
...
To set up a rSPDE model, all we need is the mesh. 
...
Notice that this function is very reminiscent of INLA's `inla.spde2.matern()` function.

This is a pattern we tried to keep consistent. All the rSPDE versions of some INLA function 
...
Let $\theta_1 = Theta1$, $\theta_2=Theta2$ and $\theta_3=Theta3$.
In terms of 
the SPDE
$$(\kappa^2 I - \Delta)^{\alpha/2}(\tau u) = \mathcal{W},$$
where $\alpha = \nu + d/2$, we have that
...
and the logit link function comes from the default choice of the prior distribution for $\nu$, which 
is beta. 

In general, for beta prior distribution for the smoothness parameter (the default)
...
proceed in the same fashion as we would in INLA's standard SPDE implementation:
...
rspde_fitprd <- inla(f.s, family = "Gamma", 
             ...
             control.predictor = list(A = inla.stack.A(stk.all),
                                      compute = TRUE, link = 1),
             control.compute = list(return.marginals = F, 
...
```{r plot_pred, fig.width=7.5, fig.height=15}
...
R-INLA implementation of the rational approximation that are not present in
INLA's standard SPDE implementation. 
...
of the precision matrix should be kept fixed during INLA's estimation and the higher
...
accurate. See the vignette [Covariance-based rational approximation with the `rSPDE` package](rspde_cov.html)
...
We can see that for this dataset, the cost of having `rspde_order=3` and 
`nu_upper_bound=2` is greater than `rspde_order=2` and `nu_upper_bound=4`:
...
Recall that for beta prior (which is the default choice of prior for $\nu$):
$$\nu = \nu_{UB}\Big(\frac{\exp(\theta_3)}{1+\exp(\theta_3)}\Big).$$
...
We begin by recalling that the fitted rSPDE model in INLA contains the 
parameters $Theta1$, $Theta2$ and $Theta3$. Let, again,
$\theta_1 = Theta1$, $\theta_2=Theta2$ and $\theta_3=Theta3$.
In terms of 
the SPDE
$$(\kappa^2 I - \Delta)^{\alpha/2}(\tau u) = \mathcal{W},$$
where $\alpha = \nu + d/2$.
...
Let us denote a beta distribution with mean $\mu$ and precision parameter $\phi$ by
$\mathcal{B}(\nu_0,\phi)$.

If we want $\nu$ to have a prior 
$$\nu \sim \mathcal{B}(\text{nu_1},\text{prec_1}),$$
one simply needs to set `prior.nu = list(mean=nu_1, prec=prec_1)`.
If one sets `prior.nu = list(mean=nu_1)`, then $\nu$ will have prior
$$\nu \sim \mathcal{B}(\text{nu_1},\phi_1),$$
where
$$\phi_1 = \max\Big\{\frac{\nu_{UB}}{\text{nu_1}}, \frac{\nu_{UB}}{\nu_{UB}-\text{nu_1}}\Big\} + \text{nu.prec.inc}.$$

Of one sets `prior.nu = list(prec=prec_1)`, then $\nu$ will have prior
$$\nu\sim \mathcal{B}(\nu_0, \text{prec_1}).$$
It is also noteworthy that if we assume beta as the prior distribution for $\nu$, we have that,
in terms of INLA's parameters,
...
in the following sense. We assume that
$$\nu = \exp(\theta_3),$$
where $\theta_3$ follow a [truncated normal distribution](https://en.wikipedia.org/wiki/Truncated_normal_distribution#One_sided_truncation_(of_upper_tail))
...
with mean $\mu$ and standard deviation $\sigma$. Then, $\theta_3$ has cumulative distribution function given by
$$F_{\theta_3}(x) = \frac{\Phi(x;\mu_0,\sigma_0)}{\Phi(\nu_{UB})},\quad x\leq \nu_{UB},$$
and $F_{\theta_3}(x) = 1$ if $x>\nu_{UB}$. We will call $\mu_0$ and $\sigma_0$ the log-location and log-scale parameters of $\nu$, respectively, and we say that
$\theta_3$ follows a truncated normal distribution with location parameter $\mu_0$ and scale parameter $\sigma_0$. 
...
The inital values to be used by INLA's optimization algorithm can be changed by
...
Notice that we sped up a lot the convergence in this case by considering a good starting value.
...
use the same example considered in the [Covariance-based rational approximation with the `rSPDE` package](rspde_cov.html) vignette (the only difference is the way the data 
is organized).
We also refer the reader to this vignette for a description of the function
`CBrSPDE.matern.operators()`, along with its methods (for instance, the `simulate` method).

### Simulating the data

Let us consider a simple Gaussian linear model with 40 independent replicates 
...
y_{i+39m} &= x_{40}(\mathbf{s}_i) + \varepsilon_{i+39m}
\end{align},
$$
where $\varepsilon_1,\ldots,\varepsilon_{2m}$ are iid normally distributed
...
that we have 40 replicates at the same locations. To this end,
...
[Covariance-based rational approximation with the `rSPDE` package](rspde_cov.html) vignette.
...
We then compute the finite element quantities (which are needed for simulation):
...
d=2
operator_information = CBrSPDE.matern.operators (C=C,
                                 G=G,
                                 nu=nu,
                                 kappa=kappa,
                                 tau=tau,
                                 ...
                                 d=d)
```
More details on this function can be found at the [Covariance-based rational approximation with the `rSPDE` package](rspde_cov.html) vignette.
...
result_df <- data.frame(true = c(tau, kappa, nu), mean = c(result_fit_rep$summary.tau$mean, 
