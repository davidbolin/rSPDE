...
For the `r inla_link()` implementation of the rational SPDE approach we refer the reader to
...
With these elements, we can use `r inla_link()` to compute the posterior distribution of the three parameters we want to estimate.
...
perform predicitions (i.e., do kringing), etc.
...
we will show how to fit parameters on such a dataset.
...
Notice that $Y$ is a matrix with 20 columns, each column containing 
