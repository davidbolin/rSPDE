---
title: "Covariance-based rational approximation with the rSPDE package"
author: "David Bolin, Alexandre B. Simas, Zhen Xiong"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Covariance-based rational approximation with the rSPDE package}
...
rational approximation is related to the operator-based [rational approximation by Bolin
and Kirchner (2020)](https://www.tandfonline.com/doi/full/10.1080/10618600.2019.1665537).

For operator-based rational approximation, see the [Operator-based rational approximation with the `rSPDE` package](rspde_base.html).

For the R-INLA implementation of the rational SPDE approach we refer the reader to
the [R-INLA implementation of the rational SPDE approach](rspde_inla.html) vignette.
...
The central idea is to perform the rational approximation directly on the
...
SPDE written above, by using the finite element method (FEM): 
$$u_h(\mathbf{s}_i)=\sum_{j=1}^{n_h} \hat{u}_j \varphi_j(\mathbf{s}_i),$$ 
where $\{\hat{u}_j\}_{j = 1}^{n_h}$ are stochastic weights and $\{\varphi_j(\mathbf{s}_i)\}_{j = 1}^{n_h}$ are fixed basis functions.
We then obtain finite element approximation of the operator $L$, which is given by $L_h$, and the covariance operator of $u_h$ is
given by $L_h^{-\alpha}$. 
...
where $m_{\alpha} = \lfloor\alpha\rfloor$,
...
Based on the above operator equation, we can write covariance matrix of the stochastic weights $\hat{\textbf{u}}$, where $\hat{\textbf{u}}=[\hat{u}_1,...,\hat{u}_{n_h}]^\top$, 
as 
...
The covariance-based method has a computational advantage when compared to
...
In this section, we explain how to to use the function `CBrSPDE.matern.operators()`
function for constructing the covariance-based rational approximation. We will
also illustrate the usage of several methods and related functions related to
the covariance-based rational approximation. We will use functions to sample from Gaussian fields
with stationary Matérn covariance function, compute the log-likelihood function,
perform predicitions (i.e., kringing), etc.
...
can be constructed, if the FEM implementation of the `R-INLA` package is used instead. 
When using the `R-INLA` package, we also recommend the usage of our `R-INLA` implementation
...
We can now use `CBrSPDE.matern.operators` to construct a rational SPDE approximation of order $m=2$ for a Gaussian random field with a Matérn covariance function on the interval. We also refer the reader to the
[Operator-based rational approximation with the `rSPDE` package](rspde_base.html) for a similar comparison made for the operator-based rational approximation.
...
can use the `precision` method on the `CBrSPDE` object:
...
```{r}
N = 10
matern.cov <- function(x,kappa,nu,sigma){
  out = 0
  for(i in -N:N){
    out <- out + matern.covariance(x + i,kappa, nu, sigma)
  }
  return(out)
}
c.true <- matern.cov(abs(s - 0.5), kappa, nu, sigma)
```
...
legend("topright", bty = "n",
       legend = c("Matérn", "m=2 CBrSPDE"),
       col = c("black", "red"),
       lty = rep(1,2), ncol = 1,
       cex = 0.8)

plot(s, c.true - c_cov.approx, type = "l", ylab = "Error", xlab = "s",
     cex.main = 0.8, cex.axis = 0.8, cex.lab = 0.8)
par(opar)
```
...
```{r}
op_cov2 <- CBrSPDE.matern.operators(kappa = kappa, tau = tau, nu = nu,
...
fem2 <- rSPDE.fem1d(s2)
op_cov <- CBrSPDE.matern.operators(kappa = kappa, tau = tau, nu = nu,
...
plot(s, c.true - c_cov.approx, type = "l", ylab = "Error", xlab = "s", col = 1,
     cex.main = 0.8, cex.axis = 0.8, cex.lab = 0.8)
...
When we use the function `CBrSPDE.matern.operators()`, we can simulate from the model using the
`simulate` method. To such an end we simply apply the simulate method to the
object returned by the `CBrSPDE.matern.operators()` function:
...
kappa = exp(theta[2])
nu = exp(theta[3])
return(-rSPDE::CBrSPDE.matern.loglike(object = op_cov, Y=Y,
...
the `get.inital.values.rSPDE()`. We then add an initial guess for `sigma.e`.

```{r, message=FALSE, warning=FALSE}
theta0 <- c(get.inital.values.rSPDE(mesh.range = 1, dim = 1),
...
print(data.frame(tau = c(tau,exp(theta$par[1])), kappa = c(kappa,exp(theta$par[2])),
...
print(data.frame(tau = c(tau,exp(theta_parallel$par[1])), 
...
Let us update the `CBrSPDE` object with the fitted parameters:
...
op_cov <- update(op_cov, user_kappa=kappa_est,
                 user_tau = tau_est,
                 user_nu = nu_est)
```
...
Let us illustrate how to simulate a dataset with replicates. Then,
we will show how to fit and do kringing on such a dataset.

Recall that to simulate a latent model with replicates, all we do is set
the `nsim` argument to the number of replicates.

Let us simulate the dataset. We will use the same `CBrSPDE` object from
...
Notice that $Y$ is a matrix with two columns, each column containing 
a replicate. Now, the remaining of the code is identical to the previous case.
The `CBrSPDE.matern.loglike()` function automatically identifies the replicates 
...
start_time <- Sys.time()
theta <- optimParallel(theta0, mlik_cov, Y = Y.rep, A = A, op_cov = op_cov,
method = "L-BFGS-B")
end_time <- Sys.time()
...
using the `R-INLA` package. Let us start by defining a mesh over $[0,1]\times [0, 1]$ 
and compute the mass and stiffness matrices for that mesh. 

It is important to mention that when using the `R-INLA` package we recommend
...
We can now use these matrices to define a rational SPDE approximation of order
...
To this end we create a model object with the `CBrSPDE.matern.operators()` function:
...
d = 2
op_cov_2d = CBrSPDE.matern.operators (C=C,
                                 G=G,
                                 nu=nu,
                                 kappa=kappa,
                                 tau=tau,
...
Now let us simulate some noisy data that we will use to estimate the parameters of the model. To construct the observation matrix, we use the `R-INLA` function `inla.spde.make.A`. We will simulate the data
with 30 replicates.
...
```{r, fig.show='hold',fig.height = 3.5, fig.width = 7, fig.align = "center",echo=FALSE}
opar <- par(mfrow = c(1,3),mgp = c(1.2, 0.5, 0), mar = c(2,2,0.5,0.5) + 0.1)
...
For each type of rational approximation of order $m$, there is a corresponding 
likelihood function that can be used for likelihood-based parameter estimation. 

Since we constructed the model with `CBrSPDE.matern.operators`, we can use the 
function `CBrSPDE.matern.loglike` to define the likelihood. 
To simplify parameter estimation, we create an objective function to maximize 
which is the negative log-likelihood, parametrized using the logarithm of each 
parameter to avoid constrained optimization. 

```{r}
mlik_2d <- function(theta, Y, A, op_cov_2d) {
  tau = exp(theta[1])
  kappa = exp(theta[2])
  nu = exp(theta[3])
  return(-rSPDE::CBrSPDE.matern.loglike(object = op_cov_2d, Y=Y,
                                  A = A, user_kappa=kappa, user_tau = tau,
                                 user_nu=nu, sigma.e = exp(theta[4])))
}    
```
...
```{r}
theta0_2d = c(get.inital.values.rSPDE(mesh=mesh_2d),
              log(0.1*sqrt(var(as.vector(Y)))))
start_time <- Sys.time()
pars <- optimParallel(theta0_2d, mlik_2d, Y = Y, A = A, op_cov_2d=op_cov_2d)
end_time <- Sys.time()
total_time <- end_time - start_time
results <- data.frame(tau = c(tau, exp(pars$par[1])), 
