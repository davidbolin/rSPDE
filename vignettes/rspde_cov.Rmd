...
The covariance-based rational approximation is an efficient alternative 
to the operator-based rational approximation when one has SPDE driven
by Gaussian white noise.

When dealing with fractional SPDE models driven by non-Gaussian white noises,
the operator-based rational approximations might be useful.
For instance, consider the SPDE: 
$$L^{\alpha/2}(\tau u) = \widetilde{\mathcal{W}},$$ 
where 
$L  = -\Delta +\kappa^2 I$ and $\widetilde{\mathcal{W}}$ is a non-Gaussian white noise. 
An example in which the operator-based rational approximation is suitable, is when one has the
above SPDE with the white noise being the so-called type G Lévy noises. 

We refer the reader to [Geostatistical Modelling Using Non-Gaussian Matérn Fields](https://onlinelibrary.wiley.com/doi/full/10.1111/sjos.12141),
[Spatial Matérn Fields Driven by Non-Gaussian Noise](https://onlinelibrary.wiley.com/doi/abs/10.1111/sjos.12046) and
[Linear mixed effects models for non-Gaussian continuous repeated measurement data](https://rss.onlinelibrary.wiley.com/doi/pdf/10.1111/rssc.12405)
...
```{r}
c.true <- folded.matern.covariance(rep(0.5,length(s)),abs(s), kappa, nu, sigma)
```
...
```{r}
op_cov2 <- matern.operators(kappa = kappa, sigma = sigma, nu = nu,
...
fem2 <- rSPDE.fem1d(s2)
op_cov <- matern.operators(kappa = kappa, sigma = sigma, nu = nu,
...
See [Operator-based rational approximation with the `rSPDE` package](rspde_base.html)
for details on the numerical instability of the operator-based rational approximation.

The main reason for the numerical stability of the covariance-based method is that
its clever
decomposition of the latent field into a vector of random fields (see the introduction for
further details) remove the need of higher order finite element matrices to have higher
...
kappa = exp(theta[2])
nu = exp(theta[3])
return(-rSPDE::rSPDE.matern.loglike(object = op_cov, Y=Y,
...
print(data.frame(sigma = c(sigma,exp(theta$par[1])), kappa = c(kappa,exp(theta$par[2])),
...
print(data.frame(sigma = c(sigma,exp(theta_parallel$par[1])), 
...
Let us simulate the dataset. We will use the same `CBrSPDEobj` object (returned
...
Let us then consider a simple Gaussian linear model with 30 independent replicates 
...
d = 2
op_cov_2d = matern.operators(mesh=mesh_2d,
                                 nu=nu,
                                 kappa=kappa,
                                 sigma=sigma,
                                 m=2)
...
mlik_2d <- function(theta, Y, A, op_cov_2d) {
  ...
  kappa = exp(theta[2])
  nu = exp(theta[3])
  return(-rSPDE::rSPDE.matern.loglike(object = op_cov_2d, Y=Y,
                                  A = A, user_kappa=kappa, user_sigma = sigma,
                                 user_nu=nu, sigma.e = exp(theta[4])))
}    
```
