...
df_data <- data.frame(y = y_vec, x_coord = loc_2d_mesh[,1],
                      y_coord = loc_2d_mesh[,2])

fit_2d <- rspde_lme(y ~ -1, model = op_cov_2d_est, data = df_data, repl = repl,
...
```{r}
print(data.frame(
  sigma = c(sigma, fit_2d$coeff$random_effects[2]), range = c(r, fit_2d$coeff$random_effects[3]),
...
```{r}
fit_ns <- rspde_lme(y ~ -1, model = op_cov_ns_est, data = data.frame(y= y, x_coord = loc_mesh[,1], y_coord = loc_mesh[,2]), loc = c("x_coord", "y_coord"))
```
...
Let us now choose some reasonable starting values (on log scale) depending on the size of the domain with the `get.initial.values.rSPDE()` function:

```{r}
theta0 <- c(get.initial.values.rSPDE(mesh = mesh, B.sigma = B.sigma, B.range = B.range), log(0.01))
```
