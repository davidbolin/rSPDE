...
We now generate the observations as $Y_i = 2 + x1 - x2 + u(s_i) + \varepsilon_i$, where $\varepsilon_i \sim N(0,\sigma_e^2)$ is Gaussian measurement noise, $x1$ and $x2$ are covariates generated by uniform and exponential distributions, respectively. We will assume that the latent process has a Matérn covariance
...
r <- sqrt(8*nu)/kappa
op_cov <- matern.operators(
  C = fem$C, G = fem$G, loc_mesh = s, nu = nu,
  range = r, sigma = sigma, d = 1, m = 2
)
...
Y <- 2 + x1 - x2  + as.vector(A %*% u + sigma.e * rnorm(n.obs))

df_data <- data.frame(y = Y, loc = obs.loc, x1 = x1, x2 = x2)
```
...
op_cov_est <- matern.operators(
  C = fem$C, G = fem$G, loc_mesh = s, d = 1, m = 2
)
```
...
```{r}
fit <- rspde_lme(y~x1+x2, model = op_cov_est,
...
```{r}
print(data.frame(
  sigma = c(sigma, fit$coeff$random_effects[2]), range = c(r, fit$coeff$random_effects[3]),
...
```{r, message=FALSE, warning=FALSE}
fit_par <- rspde_lme(y~x1+x2, model = op_cov_est,
                    data = df_data, loc = "loc", parallel = TRUE, improve_hessian = TRUE)
```
...
```{r}
print(data.frame(
  sigma = c(sigma, fit_par$coeff$random_effects[2]), range = c(r, fit_par$coeff$random_effects[3]),
  nu = c(nu, fit_par$coeff$random_effects[1]),
  row.names = c("Truth", "Estimates")
))

# Total time
print(fit_par$fitting_time)
```
...
df_data <- data.frame(y = y_vec, loc = rep(obs.loc, n.rep))
```
...
```{r}
print(data.frame(
  sigma = c(sigma, fit_repl$coeff$random_effects[2]), range = c(r, fit_repl$coeff$random_effects[3]),
  nu = c(nu, fit_repl$coeff$random_effects[1]),
  row.names = c("Truth", "Estimates")
))
...
We can obtain better estimates of the Hessian by setting `improve_hessian` to `FALSE`, however this might make the process take longer:
...
```{r}
print(data.frame(
  sigma = c(sigma, fit_2d$coeff$random_effects[2]), range = c(range, fit_2d$coeff$random_effects[3]),
  nu = c(nu, fit_2d$coeff$random_effects[1]),
  row.names = c("Truth", "Estimates")
))
...
```{r}
fit_ns <- rspde_lme(y ~ -1, model = op_cov_ns_est, data = df_data_ns, loc = c("x_coord", "y_coord"), parallel = TRUE)
```
...
```{r}
print(data.frame(
  theta1 = c(true_theta[1], fit_ns$coeff$random_effects[2]), theta2 = c(true_theta[2], fit_ns$coeff$random_effects[3]),
  theta3 = c(true_theta[3], fit_ns$coeff$random_effects[4]), 
  nu = c(nu, fit_ns$coeff$random_effects[1])),
  row.names = c("Truth", "Estimates")
)
```
...
```{r}
fit_order1 <- rspde_lme(y ~ -1, model = op_cov_2d_type, data = df_data_2d,repl = repl,
          loc = c("x_coord", "y_coord"), parallel = TRUE)
```
...
```{r}
print(data.frame(
  sigma = c(sigma, fit_order1$coeff$random_effects[2]), range = c(range, fit_order1$coeff$random_effects[3]),
  nu = c(nu, fit_order1$coeff$random_effects[1]),
  row.names = c("Truth", "Estimates")
))
```
...
```{r}
fit_brasil <- rspde_lme(y ~ -1, model = op_cov_2d_type, data = df_data_2d,repl = repl,
          loc = c("x_coord", "y_coord"), parallel = TRUE)
```
...
```{r}
print(data.frame(
  sigma = c(sigma, fit_brasil$coeff$random_effects[2]), range = c(range, fit_brasil$coeff$random_effects[3]),
  nu = c(nu, fit_brasil$coeff$random_effects[1]),
  row.names = c("Truth", "Estimates")
))
```
