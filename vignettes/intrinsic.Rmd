...
The most general intrinsic model is defined as  
$$
...
by performing two rational approximations, one for each fractional operator. The package also contains a separate implementation for the special case 
$$
...
of the general model. One can similarly use `intrinsic.operators()` to construct the `rSPDE` representation 
of the special case with $\alpha = 0$.
...
## Fitting fractional models with `R-INLA` 

Although the general intrinsic model currently only can be fitted to data in `R-INLA` having fixed values of 
$alpha$ and $\beta$, the special case with $\alpha=0$ can be estimated with general smoothness parameter. We now illustrate this. We start by simulating some data using the `intrinsic.operators()` specification
...
```
We now generate some data for this model as above. 
...
We will now fit the model using our `r inla_link()` implementation of the
rational SPDE approach. As we will estimate the smoothness, we have to use the `rspde.make.index` and `rspde.make.A` functions when creating the stack.
...
```{r}
rspde_model <- rspde.intrinsic(mesh = mesh_2d)
