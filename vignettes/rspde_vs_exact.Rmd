...
has_flexiblas <- function() {
  if (!requireNamespace("flexiblas", quietly = TRUE)) return(FALSE)
  out <- try(flexiblas::flexiblas_list(), silent = TRUE)
  !inherits(out, "try-error")
}

list_backends <- function() {
  if (!has_flexiblas()) return(character())
...
set_backend <- function(backend_name) {
  if (!has_flexiblas()) return(invisible(NULL))
...
backend_label <- function(name) {
  ...
  if (grepl("mkl", name, ignore.case = TRUE)) return("MKL")
  if (grepl("netlib", name, ignore.case = TRUE)) return("Netlib")
  if (grepl("veclib|vec|apple", name, ignore.case = TRUE)) return("VecLib")
  name
}
