...
### Spatial Example with `bounded_rho = FALSE`

First, the `rspde_lme()` implementation:

```{r}
op_2d_unbounded <- spacetime.operators(mesh_space = mesh_2d, time_loc = t,
                          kappa = kappa, sigma = sigma, alpha = alpha,
                          beta = beta, rho = rho_2d, gamma = gamma,
                          bounded_rho = FALSE)

res_unbounded <- rspde_lme(Y ~ 1, loc = c("x", "y"), loc_time = "t", data = df_2d, 
                          model = op_2d_unbounded, parallel = TRUE)

results_unbounded <- data.frame(kappa = c(kappa, res_unbounded$coeff$random_effects[1]), 
                      sigma = c(sigma, res_unbounded$coeff$random_effects[2]),
                      gamma = c(gamma, res_unbounded$coeff$random_effects[3]), 
                      rho_1 = c(rho_2d[1], res_unbounded$coeff$random_effects[4]),
                      rho_2 = c(rho_2d[2], res_unbounded$coeff$random_effects[5]),
                      sigma.e = c(sigma.e, res_unbounded$coeff$measurement_error),
                      intercept = c(0, res_unbounded$coeff$fixed_effects),
                      row.names = c("True", "Estimate"))
...
```{r}
### Fit with bounded_rho = FALSE
st_bru_spatial_unbounded <- rspde.spacetime(
  mesh_space = mesh_2d, mesh_time = mesh_time, alpha = 1, beta = 1, 
  bounded_rho = FALSE
)

### Fitting the Model
cmp_spatial_unbounded <- y ~ -1 + Intercept(1) + field(list(space = cbind(coord_x, coord_y), 
                          time = time), model = st_bru_spatial_unbounded)

bru_fit_spatial_unbounded <- bru(cmp_spatial_unbounded, data = df_bru)

### Extract and Compare Results
param_spatial_unbounded <- transform_parameters_spacetime(
                    bru_fit_spatial_unbounded$summary.hyperpar$mean[2:6], 
                    st_bru_spatial_unbounded)

results_spatial_unbounded <- data.frame(
  kappa = c(kappa, param_spatial_unbounded$kappa),
  sigma = c(sigma, param_spatial_unbounded$sigma),
  gamma = c(gamma, param_spatial_unbounded$gamma),
  rho_1 = c(rho_2d[1], param_spatial_unbounded$rho),
  rho_2 = c(rho_2d[2], param_spatial_unbounded$rho2),
  sigma.e = c(sigma.e, sqrt(1 / bru_fit_spatial_unbounded$summary.hyperpar$mean[1])),
  intercept = c(0, bru_fit_spatial_unbounded$summary.fixed$mean),
  row.names = c("True", "Estimate")
)

print(results_spatial_unbounded)
```
