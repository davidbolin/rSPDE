...
```{r}
mesh_1d_time <- fmesher::fm_mesh_1d(df_1d$time)
mesh_1d_space <- fmesher::fm_mesh_1d(df_1d$space)
```
...
```{r}
bru_fit <- bru(cmp, data = df_1d, options = list(num.threads = "15:1"))
```
...
mesh_2d_time <- fmesher::fm_mesh_1d(df_2d$t)
```
...
```{r}
st_bru_field <- rspde.spacetime(mesh_space = mesh_2d_spatial,
...
Now, the component:
```{r}
cmp <- y ~ -1 + Intercept(1) + field(list(space=cbind(x, y), 
                time = t), model = st_bru_field)
```

We are now in a position to fit the model:

```{r}
bru_fit_field <- bru(cmp, data = df_2d, options = list(num.threads = "15:1"))
```
...
```{r}
op_1d_unbounded <- spacetime.operators(space_loc = df_1d$space, time_loc = df_1d$time,
                           alpha = 1, beta = 1, 
                          bounded_rho = FALSE)
...
```{r}
### Fit with bounded_rho = FALSE
st_bru_unbounded <- rspde.spacetime(space_loc = df_1d$space, time_loc = df_1d$time, alpha = 1, 
...
bru_fit_unbounded <- bru(cmp_unbounded, data = df_1d, options = list(num.threads = "15:1", verbose=TRUE))
