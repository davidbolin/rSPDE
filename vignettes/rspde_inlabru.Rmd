...
nu <- 0.8
true_theta <- c(0,1, 1)
B.tau = cbind(0, 1, 0, (mesh$loc[,1] - 5) / 10)
B.kappa = cbind(0, 0, 1, (mesh$loc[,1] - 5) / 10)

# SPDE model
rspde_nonstat_sample <- rspde.matern(mesh, 
  B.tau = B.tau, 
  B.kappa = B.kappa,
  start.theta = true_theta, parameterization = "spde",
  nu = nu)
```
...
```{r}
start_theta <- rspde_nonstat_sample$start.theta

# Adding the first coordinate

start_theta <- c(1, start_theta)

tau <- exp(B.tau%*%start_theta)
kappa <- exp(B.kappa%*%start_theta)

ns_op <- spde.matern.operators(kappa = kappa, tau = tau, nu = 0.8,
                                mesh = mesh, m = 2, type = "covariance")
...
```{r, warning=FALSE}
u <- as.vector(inla.qsample(1, prec_mat, seed = 123))
```
