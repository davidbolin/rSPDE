...
To perform cross-validation, we create a list with the fitted models, and we pass this list to the `cross_validation()` function. It is also important to create a named list, so that the output has meaningful names for the models. We will perform a `leave percentage out` cross-validation, with the default that fits the model on 30% of the data, to predict 70% of the data.
