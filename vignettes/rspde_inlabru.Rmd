...
We use the basis function representation of $x(\cdot)$ to define the $A$ matrix
linking the point locations to the mesh.
...
We will assume $\nu = 0.8$, $\theta_1 = 1, \theta_2 = -1$ and $\theta_3=0.5$. Let us now build the model with the `rspde.matern()` function:
...
```{r}
u <- as.vector(inla.qsample(1, prec_mat))
```

Let us now obtain 200 random locations on the rectangle and 
