...
$$
Here $\boldsymbol{\mathrm{A}}$ is an identity matrix since we are evaluating the approximation in the nodes of the FEM mesh and $\boldsymbol{\mathrm{v}}$ is a vector with all basis functions evaluated in $s=0.5$. This way of computing the covariance is obtained by setting `direct = TRUE` in `cov_function_mesh()`:
```{r}
...
Let us fit again and check the results. We use the previous fit to speed up the computation.
