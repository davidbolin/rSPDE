...
We can now obtain the objective function to minimize, which is the negative log-likelihood, parametrized using the logarithm of 
each parameter to avoid constrained optimization, using the `rSPDE.construct.matern.loglike()` function factory:
```{r}
mlik <- rSPDE.construct.matern.loglike(op_obj, Y=Y, A=A)
```
